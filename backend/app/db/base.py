...
if DATABASE_URL:
    ...
    if db_user and db_password and db_name and db_host:
        ...
        print("   Required: Either DATABASE_URL or all of (DB_HOST, DB_USER, DB_PASSWORD, DB_NAME)")
...
def get_engine(url, retries=10, delay=5):
    ...
    for i in range(retries):
        try:
            # Pre-check TCP connectivity (only on first attempt or after long wait)
    ...
    raise Exception(f"Could not connect to database after {retries} retries. Last error: {last_error}")
